module.exports = (Xpix, Ypix) => {
	...
	let symbols = fs.readFileSync(path.join(__dirname, 'symbols.json'))
	...
	const dateLayer = {
		yOffset: 61,
		symbolWidth: symbols.a[0].length,
		symbolHeight: symbols.a.length,
		...
		getDateLayer(d) {
			...
			dateString = dateString.match(/^[a-z]{3}\ [a-z]{3}\ \d*/i)[0]
			dateString = dateString.replace(/\ /g, '')
			dateString = dateString.toLowerCase()

			for (let i = 0; i < dateString.length; i++) {
				const symbol = symbols[dateString[i]]
				for (let y = 0; y < symbol.length; y++) {
					for (let x = 0; x < symbol[y].length; x++) {
						canvas[y + this.yOffset][x + (i * (this.symbolWidth + 1))] = symbol[y][x]
					}
				}
			}
			return canvas
